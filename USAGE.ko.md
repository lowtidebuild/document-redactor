...
- **`document-redactor.html`** — 도구 본체 (271,511 bytes, under 300 KB, HTML 한 파일)
...
**Export policy** 로 현재 수동 추가 항목과 선택/해제 기본값을 로컬 JSON 파일로 저장할 수 있습니다. **Import policy** 는 그 JSON을 다른 문서에 적용합니다. 정책 파일에는 문자열, 카테고리, 기본 선택값만 들어가며 DOCX 내용은 들어가지 않습니다.
...
한국 사건번호 (`2024가합12345`), `Case No.: 24-CV-1234`, `Docket No.: 1:24-cv-00001`, `사건번호:` 라벨처럼 문서를 특정 사건이나 도켓에 연결할 수 있는 식별자. **기본 언체크** — 해당 참조 자체를 숨겨야 할 때만 opt-in 합니다.

계약 조항 참조 (`Section 10.1`, `제15조 제2항`), 법원명 (`서울중앙지방법원`, `대법원`), 판례 인용 (`123 F.3d 456`), 공개 법령 인용 (`민법 제750조`, `17 U.S.C. § 101`) 은 기본적으로 보존됩니다. 보통 민감정보가 아니라 문서 구조, 관할/장소 맥락, 공개 법적 근거이기 때문입니다.
...
- **8개 카테고리 섹션은 자동 감지 결과로 채워집니다.** 주소, 전화, 이메일, ID, 금액, 날짜는 기본 체크됩니다. 사건번호와 도켓 라벨은 검토용으로 표시될 수 있지만 기본 언체크이며, 계약 조항, 법원명, 판례 인용, 공개 법령 인용은 직접 추가하지 않는 한 보존됩니다.
